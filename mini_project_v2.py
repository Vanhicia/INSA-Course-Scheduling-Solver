...
def get_model (N):
    ...
    course_1 = 40  # Maths
    course_2 = 30  # CS, in cs_rooms
    course_3 = 10  # Chemistry
    course_4 = 20  # English
    course_5 = 10 # ppi
    lab_1 = 15  # CS labs
    ...
    planning_course = Matrix(len(course_list), number_of_weeks, 1, limit_hours_course)
    planning_labs = Matrix(len(lab_list), number_of_weeks, 1, limit_hours_course)

    model = Model(

        # Courses
        [Sum(row) == hours for (row, hours) in zip(planning_course.row, course_list)],
